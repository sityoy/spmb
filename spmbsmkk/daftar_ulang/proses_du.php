...
if (isset($_POST['simpan_du'])) {
    ...
    // LOGIKA ANTI-ERROR (Cek langsung ke tabel daftar_ulang)
    $cek_tabel_du = mysqli_query($conn, "SELECT id_pendaftar FROM daftar_ulang WHERE id_pendaftar = '$id'");
    
    if (mysqli_num_rows($cek_tabel_du) > 0) {
        // Jika data sudah ada (hasil dari 1-klik / sebelumnya), lakukan UPDATE
    ...
    // Kunci status utama pendaftar menjadi Sudah Daftar Ulang
...
<head>
    <meta charset="UTF-8">
    <title>Proses Daftar Ulang - <?php echo htmlspecialchars($data['nama_lengkap']); ?></title>
    <style>
...
<body>
    <div class="box">
        ...
        <div class="identitas">
            <b style="font-size:16px;"><?php echo htmlspecialchars($data['nama_lengkap']); ?></b><br>
            <span style="font-size:13px; color:#475569;">No: <?php echo htmlspecialchars($data['no_pendaftaran']); ?> | NISN: <?php echo htmlspecialchars($data['nisn']); ?></span><br>
            <span style="font-size:13px; font-weight:bold; color:#d97706;">Jurusan: <?php echo htmlspecialchars($data['pilihan_jurusan']); ?></span>
        </div>
